...
const canShortUsed = async (ctx, next) => {
  ...
  try {
    ...
    if (shortRd) {
      ...
      shortRd.visitCount++;
      await rd.setObj(`short:${short}`, shortRd, SHORT_EXPIRT_TIME);
      ...
      if (!res) {
        ...
        res.visitCount++;
        await rd.setObj(`short:${short}`, res, SHORT_EXPIRT_TIME);
        ctx.state.short = res;
      }
    }
    ...
    async function isAllow(st) {
      // 是否过期
      if (!st.isEver && (new Date() - SHORT_EXPIRT_TIME) > new Date(st.create_time)) {
